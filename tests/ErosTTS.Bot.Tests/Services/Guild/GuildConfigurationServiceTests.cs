...
public class GuildConfigurationServiceTests
{
    ...
    private readonly GuildConfigurationService _sut;

    public GuildConfigurationServiceTests()
    {
        _logger = Substitute.For<ILogger<GuildConfigurationService>>();
        _sut = new GuildConfigurationService(_logger);
    }
    ...
    [Fact]
    public async Task ConcurrentSetAndGet_WithMultipleGuilds_IsThreadSafe()
    {
        var tasks = Enumerable.Range(1, 100)
            .Select(i => Task.Run(async () =>
            {
                var guildId = (ulong)i;
                await _sut.SetChannelsAsync(guildId, guildId * 10, guildId * 100);
                var config = await _sut.GetConfigurationAsync(guildId);
                return config;
            }))
            .ToList();
        ...
        var allConfigs = await _sut.GetAllConfigurationsAsync();
        allConfigs.Should().HaveCount(100);
    }
