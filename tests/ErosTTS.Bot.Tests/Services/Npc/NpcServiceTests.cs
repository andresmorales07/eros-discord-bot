...
{
    private readonly NpcService _sut;

    public NpcServiceTests()
    {
        var logger = Substitute.For<ILogger<NpcService>>();
        ...
        _sut = new NpcService(config, logger);
    }
    ...
    public async Task SetHistoryModeAsync_NoChangeDoesNotClearHistory()
    {
        ...
        // Default is false, setting to false again should not clear
    ...
    public async Task ImportNpcsAsync_SkipsExistingNames()
    {
        ...
        // Original Gandalf unchanged
        var gandalf = await _sut.GetNpcAsync(1UL, "Gandalf");
        gandalf!.Personality.Should().Be("Existing wizard");
    }

    [Fact]
    public async Task ImportNpcsAsync_InvalidVersion_Throws()
    {
        var json = """{ "version": 99, "npcs": [] }""";

        var act = async () => await _sut.ImportNpcsAsync(1UL, json);
    ...
    [Fact]
    public async Task ImportNpcsAsync_EmptyNpcs_Throws()
    {
        var json = """{ "version": 1, "npcs": [] }""";
